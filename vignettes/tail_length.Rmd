...
rowData(cal_design) %>% as.data.frame() %>%
    ggplot(aes(x=log2(total_reads), y=dispersion_unweighted^0.25)) + geom_point(size=0.1) +
    labs(title="Dispersion^0.25 if uniform weights used")
```
...
rowData(cal_design) %>% as.data.frame() %>%
    ggplot(aes(x=log2(total_reads), y=dispersion_weighted^0.25)) + geom_point(size=0.1) +
    geom_line(aes(y=dispersion_trend^0.25), color="red") +
    labs(title="Dispersion^0.25 with read-counts as weights")
```
...
```{r disp-plot-cal}
rowData(cal_design) %>% as.data.frame() %>%
    ggplot(aes(x=log2(total_reads), y=dispersion_calibrated^0.25)) + geom_point(size=0.1) +
    labs(title="Dispersion^0.25 with read-count weights / trend-based calibration")
```
