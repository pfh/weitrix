---
title: "RNA-Seq componenents (airway dataset)"
...
design <- model.matrix(~ dex + cell, data=colData(airway_good))
...
* `vooma` provides precision weights. One could instead choose a `prior.count` for `cpm` to produce a flat mean-variance trend-line.
...
Without varimax rotation the treatment effect is still mostly in the first component, but has also leaked into the other components.
...
```{r}
fit <- lmFit(airway_elist, comp$col) %>% eBayes()
