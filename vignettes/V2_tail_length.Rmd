...
To make this a bit more concrete, we can estimate how much extra variation there is per observation not explained by the model. The function `weitrix_rms_confects` provides this, and further provides a confident lower bound on this excess with a multiple testing correction. The "effect" column in the output is excess variation, and the "confect" column is a confident lower bound on this. Results are sorted by confect. The units are root-(weighted)mean-square variation, i.e. in the same units as the observations themselves.

```{r excess, warning=F}
confects <- weitrix_rms_confects(cal, ~1)
confects
```

```{r excess2, echo=FALSE, fig.show="hold", fig.width=3, fig.height=3}
...
for(id in confects$table$name[1:5]) print(view_gene(id))
```
...
```{r excess3, warning=F}
confects2 <- weitrix_rms_confects(cal, ~time)
confects2
```

```{r excess4, echo=FALSE, fig.show="hold", fig.width=3, fig.height=3}
...
for(id in confects2$table$name[1:5]) print(view_gene(id))
```
