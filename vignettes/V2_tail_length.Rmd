...
```{r examine, fig.show="hold", fig.width=3, fig.height=3}
...
Genes with high dispersion (weighted residual variance) are often of biological interest. Here we examine dispersion relative to a simple model with only an intercept term.

```{r overdisp1}
dispersion <- weitrix_dispersions(cal, ~1)

# Sort genes by dispersion
df <- rowData(cal)
df$dispersion <- dispersion
df <- df[order(-df$dispersion),]
head(df, 10)
```

```{r overdisp2, echo=FALSE, fig.show="hold", fig.width=3, fig.height=3}
plot(dispersion ~ total_reads, data=df, log="x", cex=0.5)
for(id in rownames(df)[1:5]) print(view_gene(id))
```

MFA1 (mating pheromone a-factor) is clearly related to the arrest of cell cycling using yeast pheromones.
...
```{r overdisp3}
dispersion2 <- weitrix_dispersions(cal, ~time)
```

```{r overdisp4, echo=FALSE, fig.show="hold", fig.width=3, fig.height=3}
# Sort genes by dispersion
df2 <- rowData(cal)
df2$dispersion <- dispersion2
df2 <- df2[order(-df2$dispersion),]
head(df2, 10)
plot(dispersion ~ total_reads, data=df2, log="x", cex=0.5)
for(id in rownames(df2)[1:5]) print(view_gene(id))
```
...
```{r examine_C1, echo=FALSE, fig.show="hold", fig.width=3, fig.height=3}
result_C1$table %>% 
...
```{r examine_C2, echo=FALSE, fig.show="hold", fig.width=3, fig.height=3}
result_C2$table %>% 
    dplyr::select(gene,loading=effect,confect,total_reads) %>% 
    head(10)
...
```{r examine_C3, echo=FALSE, fig.show="hold", fig.width=3, fig.height=3}
result_C3$table %>% 
    dplyr::select(gene,loading=effect,confect,total_reads) %>% 
    head(10)
